/**
 *	Bugle-js => event-driven publish-subscribe javascript api
 *
 *	TODO: 
 *		1. reveal subscriber pipe api to user, 
 *		2. chain calls until event is published to subscriber
 **/
;(function (module) {
	...
	_flatMap = function(fn, arr) {
		// map array over accumulator function, 
		// flatten each message item but no join

		return _compose.call(
			this, 
			_map.bind(null, fn), 
			_map.bind(null, _flatten)
		)(arr);
	},

	//+ _done :: function,array -> unit
	_done = function(fn, arr) {
		fn.apply(this, [].concat(arr));
	},

	// pipe api module
	// functions are captured during pipe phase
	// apply collections over functions during digest phase
	_pipeEvents = {

		'map': function(fn) {
			return function(arr) {
				return _map.call(this, fn, arr);
			};
		},
		
		'filter': function(pred) {
			return function(arr) {
				return _filter.call(this, pred, arr);
			};
		},

		'flatMap': function(fn) {
			return function(arr) {
				return _flatMap.call(this, fn, arr);				
			};
		},
