;(function (module) {
	...
	_compose = function(fns) {
		return function(arr, scope) {
			return fns.reduce(function(iter, fn, index) {
				return fn.call(scope, iter);
			}.bind(this), arr);
		};
	},
	...
	_flatten = function(arr) {
		return arr.reduce(function(iter, curr) {
			var isArr = _truthy(curr).array();
			return iter.concat(isArr? _flatten(curr) : curr);
		}.bind(this), []);
	},
	...
	_map = function(fn) {
		return function(arr) {
			return arr.reduce(function(iter, curr) {
				return iter.concat([fn.call(this, curr)]);
			}.bind(this), []);
		};
	},
	...
	_filter = function(pred) {
		return function(arr) {
			var selected = arr.reduce(function(iter, curr) {
				return pred.call(this, curr)? iter.concat(curr): iter;
			}.bind(this), []);
	...
	_reject = function(pred) {
		...
		return function(arr) {
			var selected = _filter(rejectBy).call(this, arr);

			// don't need to wrap in array here, filter does this for us
			return selected;
		}
	},
	...
	_flatMap = function(fn) {
		...
		return function(arr) {
			var flattened = _compose(toCompose)(arr, this);
			return [flattened];
		};
	},

	//+ _reduce :: (function,any) -> any
	_reduce = function(fn, iter) {

		return function(arr) {
			var reduced = arr.reduce(function(iter, curr) {
				return fn.call(this, curr, iter);
			}.bind(this), iter? iter : []);
	...
	_reduceRight = function(fn, iter) {
		return function(arr) {
			arr.reverse();
			return _reduce(fn, iter).call(this, arr);
		}
	},
	...
	Subscriber.prototype = {
		...
		'reduce': function(fn) {
			this._pipeline_.push(_reduce(fn));
			return this;
		},

		//+ reduceRight :: function -> Subscriber
		'reduceRight': function(fn) {
			this._pipeline_.push(_reduceRight(fn));
			return this;
		},
